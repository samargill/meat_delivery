<?php
	...
	if (isset($_REQUEST['cboClientType']))
		$cboAdminType = $_REQUEST['cboClientType'];
	else
		$cboAdminType = 0;
	...
	if (isset($_REQUEST['Delete']))
	{
		$Query = "DELETE FROM clientuser".
			" WHERE userid = ".$_REQUEST['UserID']." AND clientid = ".$_SESSION[SessionID."ClientID"];
...
<head>
	...
	<script language="javascript">
		...
		function EditRights(AdminID)
		{
			var Win = Popup("userrights?AdminID="+AdminID,"KS_PrimeMedic_Edit",740,1024,100,100);
			Win.focus();
		}
		function EditUser(AdminID)
		{
			var Win = Popup("useradd?AdminID="+AdminID,"KS_PrimeMedic_Edit",740,1024,100,100);
			Win.focus();
		}
				...
				$.confirm({
					...
					buttons: {
						"confirm": {
							...
							keys: ['enter'],
							action: function() {
								window.location = "userview?Delete&AdminID="+AdminID;
								return(false);
							}T
						},
						"cancel": {
					...
					<form name="Form" role="form" action="userview" method="post">
						<div class="card-body">
									...
									<div class="form-group">
										...
										<?php
											DBCombo("cboClientType","admintype","admintypeid","admintypename",
												"WHERE 1 ORDER BY admintypeid",$cboAdminType,"--- Show All ---","form-control select2",
							...
							<table id="MyDataTable" class="table table-bordered table-hover">
								...
								<?php
									$Index = 0;
									$Query = "SELECT C.adminid, CT.admintypename, C.firstname,".
										" C.email, C.mobile, C.lastlogin,".
										" TIMESTAMPDIFF(MINUTE,C.lastactive,NOW()) As TimeDiff".
										" FROM adminlogin C".
										" INNER JOIN admintype CT ON C.admintype = CT.admintypeid".
										" WHERE C.adminid = ".$_SESSION[SessionID."Admin"];
									...
									if ($cboAdminType > 0)
									{
										$Query .= " AND C.admintype = ".$cboAdminType;
									}
									if ($ChkDate > 0)
									{
								...
								?>
									<tr>
										...
										<td align="left"  ><?php echo($objRow->admintypename);?></td>
										<td align="left"  ><?php echo(UCString(trim($objRow->firstname)));?></td>
										...
										<td align="center">
											<div class="btn-group">
												<button type="button" onclick="EditRights(<?php echo($objRow->adminid);?>);" class="btn bg-purple btn-sm" data-toggle="tooltip" data-container="body" title="User Rights">
													<i class="fa fa-unlock"></i>
												</button>
												<button type="button" onclick="EditUser(<?php echo($objRow->adminid);?>);" class="btn btn-warning btn-sm" data-toggle="tooltip" data-container="body" title="Edit">
													<i class="fa fa-edit"></i>
												</button>
												<button type="button" onclick="DeleteUser(<?php echo($objRow->adminid);?>);" class="btn btn-danger btn-sm" data-toggle="tooltip" data-container="body" title="Delete">
