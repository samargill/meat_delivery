<?php
	...
	if (isset($_POST['SmsID']))
	{
		...
		$Query = "DELETE FROM optout".
			" WHERE mobileid = ".$objRow->mobileid." AND mobile = ".$objRow->mobile;
		mysqli_query($Conn,$Query);
		$Query = "UPDATE smsreclist SET optout = 0 WHERE smsid = ".$_REQUEST['SmsID'];
		mysqli_query($Conn,$Query);
		if (mysqli_errno($Conn) > 0)
		{
			$RespHead = "Error";
			$RespText = "Unable To Opt-in The Sender<br><br>".mysqli_error($Conn)."<br><br>".$Query;
		}
		else
		{
			$RespHead = "Done";
			$RespText = "Sender Added To Opt-in List Successfully.";
		}
	}
Response:
