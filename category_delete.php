<?php
	...
	if (isset($_POST['Validate']))
	{
		...
		$Query = "SELECT product_id FROM product".
			" WHERE category_id = ".$_POST['CategoryID']." LIMIT 0,1";
...
Response:
	...
	elseif (isset($_POST['Apply']))
	{
		if ($_POST['Apply'] == "Yes")
		{
			...
			$Photo = GetValue("photo", "product_category", "category_id = ".$_POST["CategoryID"]);
			$PhotoPath 	= $PagePath."../assets/images/category/";
			@unlink($PhotoPath.$Photo);
			// Delete Category
			$Query = "DELETE FROM product_category WHERE category_id = ".$_POST['CategoryID'];
		...
		{
			$Query = "UPDATE product_category SET status = 0, deletedate = NOW() WHERE category_id = ".$_POST['CategoryID'];
			mysqli_query($Conn,$Query);
			if (mysqli_errno($Conn) == 0)
			{
				$RespHead = "Done";
